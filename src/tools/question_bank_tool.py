...
回答要简洁明了，适合学生学习理解。"""
        ...
        if similar_questions:
            ...
            result += f"为了帮助你巩固这个知识点，老师为你准备了 **{len(similar_questions)}** 道相似题目，建议你先自己尝试解答，再对照答案。\n\n"
            ...
            result += f"3. **理解错题**：如果有错题，查看解析理解原因\n"
            result += f"4. **举一反三**：尝试自己设计一道类似题目\n"
            result += f"5. **及时提问**：如果还有疑问，随时向老师提问\n\n"
            result += f"🎯 **目标**：通过练习相似题型，加深对知识点的理解和应用能力！"
        else:
            result += f"\n{'─'*60}\n"
            result += f"## 📚 相似题型推荐\n\n"
            result += f"⚠️ 题库中暂时没有找到与你这道题相似的练习题目。\n\n"
            result += f"💡 **建议**：\n"
            result += f"1. 将这道题收藏起来，下次复习\n"
            result += f"2. 尝试自己设计一道类似题目\n"
            result += f"3. 向老师索要更多相关练习题\n"
